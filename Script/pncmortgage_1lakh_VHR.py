...
class App:
    ...
    def __init__(self, url='https://www.pnc.com/en/personal-banking/borrowing/home-lending/mortgages/VA-Loan.html',
                 loan_amount=100000, zipcode='10004'):
    ...
    def data_page(self):
        ...
        for pro in pro_name:
            if pro.getText() is not None:
                # print(pro.getText())
                Term.append(pro.getText().rstrip("Yr Fixed \xa0 ").lstrip('VA'))
...
if __name__ == '__main__':
    ...
    df = df.iloc[0:2,:]
    df.to_csv(output_path + "PNC_Data_Mortgage_1lakh_VHR.csv".format(now.strftime("%m_%d_%Y")), index=False)

    df1 = pd.read_csv(output_path+"PNC_Data_Mortgage_1lakh_FixedR.csv")
    df2 = pd.read_csv(output_path+"PNC_Data_Mortgage_1lakh_AdjustR.csv")
    df3 = pd.read_csv(output_path+"PNC_Data_Mortgage_1lakh_FHAR.csv")
    df4 = pd.read_csv(output_path+"PNC_Data_Mortgage_1lakh_VHR.csv")
    df = pd.concat([df1, df2, df3, df4])
    df.to_csv(output_path + "PNC_Data_Mortgage_1LAKH_complete.csv".format(now.strftime("%m_%d_%Y")), index=False)