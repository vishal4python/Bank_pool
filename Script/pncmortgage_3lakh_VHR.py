...
if __name__ == '__main__':
    ...
    df = df.iloc[0:2,:]
    df.to_csv(output_path + "PNC_Data_Mortgage_3lakh_VHR.csv".format(now.strftime("%m_%d_%Y")), index=False)

    df1 = pd.read_csv(output_path+"PNC_Data_Mortgage_3lakh_FixedR.csv")
    df2 = pd.read_csv(output_path+"PNC_Data_Mortgage_3lakh_AdjustR.csv")
    df3 = pd.read_csv(output_path+"PNC_Data_Mortgage_3lakh_FHAR.csv")
    df4 = pd.read_csv(output_path+"PNC_Data_Mortgage_3lakh_VHR.csv")
    df = pd.concat([df1, df2, df3, df4])
    df.to_csv(output_path + "PNC_Data_Mortgage_3LAKH_complete.csv".format(now.strftime("%m_%d_%Y")), index=False)